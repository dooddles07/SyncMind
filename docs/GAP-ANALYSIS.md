...
| Backend | Schema live, client factories live, auth live, real upload pipeline live (create meeting, chunk, upload, finalize). No transcription/AI pipeline yet — that's M2. |
...
| 1.4 | ~~`app/api/**` has one route~~ **Partial** | `POST /api/meetings` and `PATCH /api/meetings/:id` (status-flip only) now real, auth-checked (401 without a session, verified by curl), thin delegators into `server/controllers/meeting-controller.ts`. `GET /api/meetings/:id/status`, `/api/pipeline/advance`, `/api/pipeline/retry` still don't exist — those are M2, need a real transcription pipeline to advance. |
