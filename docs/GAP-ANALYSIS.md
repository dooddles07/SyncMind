...
| Backend | Schema live, client factories live, auth live, real upload pipeline live, real transcription pipeline live (quota, Groq client, `/api/pipeline/advance`). Only minutes/actions/email analysis (M3) is still unbuilt. |
| Auth | Does not exist. No `middleware.ts`, no session, no route protection. |
| AI | Transcription real (Groq Whisper, `lib/quota.ts`). No Gemini fallback client, no analysis/minutes/actions prompts, no Zod schemas yet. |
...
| 1.6 | ~~No AI layer~~ **Transcription done, analysis still open** | `server/config/groq.ts` (raw `fetch`, no SDK dependency — Groq's REST API doesn't need one for one endpoint), full retry ladder from `ARCHITECTURE.md` §7 (429 short-retry-or-quota-block, 5xx exponential backoff 2s/6s/18s). `server/utils/transcript-stitch.ts` — the offset-shift/seam-dedup arithmetic, real unit tests with synthetic fixtures as the doc specifically asked for. Minutes/actions/email (needs Zod schemas + structured LLM output) is still fully open — separate, later chunk. |
