...
| Backend | Schema live, client factories live, auth live, real upload pipeline live, real transcription pipeline live, real analysis pipeline live (single-pass minutes + action items). Only the email draft and "Ask this meeting" (rest of M3) are still unbuilt. |
| Auth | Live — Google OAuth, `middleware.ts`, session, route protection. |
| AI | Transcription real (Groq Whisper). Analysis real (Groq Llama 3.3 70B, single-pass only — map-reduce and the Gemini fallback are deliberately deferred). No email-draft or ask-this-meeting prompts yet. |
...
| 1.6 | ~~No AI layer~~ **Transcription + analysis done, email/ask still open** | `server/config/groq.ts` (raw `fetch`, no SDK dependency — Groq's REST API doesn't need one for two endpoints), full retry ladder from `ARCHITECTURE.md` §7. `server/utils/transcript-stitch.ts` — offset-shift/seam-dedup arithmetic, unit tested. `server/controllers/analysis-controller.ts` — single-pass minutes + action items via Zod-validated structured output (`server/config/analysis-schema.ts`, `server/utils/structured-output.ts`), verified live against a real meeting. Map-reduce (needed for meetings over ~5k transcript tokens) and the Gemini fallback are both deliberately deferred, flagged not hidden. Email draft and "Ask this meeting" are still fully open — separate, later chunks. |
