...
| Backend | Schema live, client factories live, auth live, real upload/transcription/analysis/email pipeline live end-to-end — a meeting can now actually reach `"ready"`. Only "Ask this meeting" (rest of M3) is still unbuilt. |
| Auth | Live — Google OAuth, `middleware.ts`, session, route protection. |
| AI | Transcription real (Groq Whisper). Analysis and email draft both real (Groq Llama 3.3 70B) — single-pass only, map-reduce and the Gemini fallback deliberately deferred. No ask-this-meeting prompt yet. |
...
| 1.6 | ~~No AI layer~~ **Transcription + analysis + email done, ask still open** | `server/config/groq.ts` (raw `fetch`, no SDK dependency), full retry ladder from `ARCHITECTURE.md` §7. `server/utils/transcript-stitch.ts` — offset-shift/seam-dedup arithmetic, unit tested. `server/controllers/analysis-controller.ts` and `server/controllers/email-controller.ts` — single-pass minutes/actions and the follow-up email draft, both Zod-validated structured output, both verified live against a real meeting that now actually reaches `meetings.status = "ready"`. `app/api/meetings/[id]/email/regenerate` lets the user re-draft in a different tone on demand. Map-reduce (needed for meetings over ~5k transcript tokens) and the Gemini fallback are both deliberately deferred, flagged not hidden. "Ask this meeting" is still fully open — separate, later chunk. |
...
| 1.10 | [app/share/[token]/page.tsx:16](<../app/share/[token]/page.tsx#L16>) ignores its token | Hardcoded to `q3-planning`, which no longer exists now that `lib/mock/data.ts` reads real data (1.9). The predicted "data leak the day it is real" didn't happen — it just renders blank (`getMeeting` returns `undefined`, page returns `null`) — the safer failure mode, but still needs its real implementation: service-role client, token lookup, `revoked_at`/`expires_at` checks per `docs/DATA-MODEL.md` §4. |
