...
| 1.4 | ~~`app/api/**` has one route~~ **Mostly done** | `POST /api/meetings`, `PATCH /api/meetings/:id`, `GET /api/meetings/:id/status`, `POST /api/pipeline/advance` all real, auth-checked, thin delegators. Only `/api/pipeline/retry` still missing — not urgent, `advance` is already safely re-callable on a `failed` meeting's remaining work in principle, a dedicated retry endpoint is a small later addition. |
