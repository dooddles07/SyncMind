...
| 2.9 | ~~No tests~~ **Further resolved, one item left** | Vitest installed, 61 real tests across every pure function in the codebase, wired into `ci.yml`. Chunker offset math: `lib/audio/chunker.ts`'s chunk-boundary loop extracted to a pure `planChunks(durationSec)`, 5 real tests in `lib/audio/chunker.test.ts` (single short chunk, exact-boundary chunk, overlap chaining across 2 and 3+ chunks, and the "never pad a chunk past the actual remaining duration" edge case). `chunkAudio` now calls `planChunks` instead of duplicating the math — behavior-preserving, confirmed via full typecheck/lint/test/build. Playwright E2E still open — needs new test infra, not a quick addition. |
