...
| Google refresh token | Postgres, AES-256-GCM encrypted | Until disconnect or account deletion |
| Email, name, avatar | Postgres, from Google sign-in | Until account deletion |
| Usage counters | Postgres, per user per day | 90 days |

We do not store Google access tokens, passwords (there are none — OAuth only), or payment data (there is none).
...
- Server-only keys (`SUPABASE_SERVICE_ROLE_KEY`, `GROQ_API_KEY`, `GEMINI_API_KEY`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_TOKEN_ENC_KEY`, `CRON_SECRET`) are referenced only from `app/api/**` and server modules. CI greps `"use client"` files for these names and fails the build on a hit.
...
Requested scopes are the minimum that make the features work:

| Scope | Why | What it cannot do |
| --- | --- | --- |
| `gmail.compose` | Create a draft in the user's Drafts | **Cannot send mail.** No send code path exists in the repo. |
| `calendar.events` | Create events for action-item due dates | Cannot read or alter calendar settings, sharing, or other calendars |
| `userinfo.email`, `userinfo.profile`, `openid` | Identity | — |

Requested incrementally: sign-in asks only for identity. Gmail and Calendar consent is requested the first time the user clicks a feature that needs it, so the initial sign-in is a low-friction, low-alarm screen.

Disconnect calls Google's revocation endpoint and deletes the stored token, rather than only clearing our row.
...
**Delete all my data** in Settings deletes every meeting, revokes the Google token, and deletes the `auth.users` row, which cascades the profile. It is behind a typed confirmation.
...
- SyncMind creates email drafts. It cannot send email.
